...
## 0. 转向决策(2026-07-17,用户拍板,当天两次)

三个已确认决策(第 1 条当晚被用户修订,见下方"晚间修订"):

1. ~~**游戏卡带全部移除**~~(git 历史留档,沿用 2026-07-17 槽位清洗先例)。tilt_maze /
   busy_knobs / chick_pour / chain_lab / fish_pond / pipe_garden 六个 app 目录整体删除;分区表/
   存储布局沿用不改,首批 IoT 应用占 ota_0/ota_1 两个槽。
...
同日晚间用户推翻决策 1:**游戏卡带不删,与 IoT 评估台共存**。四张已实现游戏(tilt_maze /
busy_knobs / chick_pour / chain_lab)从删除前的提交(`0266fe1^`)原样恢复,只补一处编译期改名
补丁(`CORE2_BOARD_CFG_KIDS_DEFAULT` → `CORE2_BOARD_CFG_DEFAULT`,配合 Phase 0 的宏改名);
fish_pond / pipe_garden 当时仅有 SPEC 未实现,不恢复。分区表不改(仍是 factory + 6 ota 槽),
槽位重排为:**ota_0=tilt_maze、ota_1=busy_knobs、ota_2=chick_pour、ota_3=chain_lab、
ota_4=unit_bench(原定 0x190000 改落 0x990000)、ota_5=power_lab(原定 0x390000 改落
0xB90000)**。app bin 不绑定槽位,换偏移单刷即可,无需改代码。此修订详见 Phase 2 §3 前置步骤。
...
## 3. Phase 2 —— 游戏回归 + 两张 IoT 卡带

### 前置步骤:游戏卡带恢复与槽表重排(晚间修订新增)

1. `git checkout 0266fe1^ -- apps/tilt_maze apps/busy_knobs apps/chick_pour apps/chain_lab`
   恢复四张已实现游戏(fish_pond/pipe_garden 当时仅有 SPEC 未实现,不恢复)。
2. 补丁:恢复出的游戏引用 `CORE2_BOARD_CFG_KIDS_DEFAULT`,已在 Phase 0 改名
   `CORE2_BOARD_CFG_DEFAULT`,逐个替换(Phase 1 其余改动均为增量 API,不影响游戏);四张游戏
   分别 build 验证,除该改名外代码逻辑不动(as-built 遗产)。
3. 槽表/文档重排为共存口径:ota_0=tilt_maze、ota_1=busy_knobs、ota_2=chick_pour、
   ota_3=chain_lab、ota_4=unit_bench(0x990000)、ota_5=power_lab(0xB90000)。改
   `tools/flash_one.sh`、`tools/flash_map.md`、`partitions.csv` 头部注释(字段不动)、
   `CLAUDE.md` §1/§9/§12、本文件 §0"晚间修订"、`README.md`/`launcher/README.md` 对应表述。
4. launcher 无需改动(数据驱动,游戏卡片按工程名自动渲染,无需为游戏加图标分支)。

### unit_bench(ota_4,外设/单元评估台)
...
在役保留(转向前):tilt_maze / busy_knobs / chick_pour / chain_lab。这四张卡带在 **2026-07-17
平台转向**当天先被整体删除、当晚又按用户修订的共存口径恢复上线(槽位从 ota_0~3→重排后仍是
ota_0~3,只是与 IoT 评估台共存而非独占)——见本文件 §0"晚间修订"与 §3 前置步骤。完整的历史
分析、候选卡带评审(pipe_garden / slingshot_feed 等)、marble_machine 否决记录等均可在 git
历史中查阅本文件转向前的版本(`git log -- docs/ROADMAP.md`)。
