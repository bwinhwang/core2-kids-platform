...
| App | 槽 | 外设 | 状态 | 文档 |
|---|---|---|---|---|
| **tilt_maze** 倾斜迷宫 | ota_0 | IMU MPU6886 | ✅ 实机验收(核心 M0–M5 + 打盹 + 难度批:陷阱/巡逻怪/绕圈怪)+ **死亡演出批**(ST_FAIL 定格 900ms + 碎片/双环/焦痕/世界震/红框告警)) | `apps/tilt_maze/SPEC.md` + `README.md` |
...
> 是否真的有功能」是立项时就该验的。**pipe_garden(2026-08-07 效果图阶段放弃,§7:
> 一屏对象太杂 + 操作只有一种「玩不出花样」)**;**候选池已空**,下一张卡带从头选题。
> ota_5 空闲;ota_3 已由 clock_turn 接管。
>
> 🔴 **新候选立项流程(ROADMAP §7 定):SPEC → 主机离屏效果图 → 过 §5 两道实现级验收 →
> 才轮到写 C。** 文字天然掩盖信息密度和重复度,画出来才知道是「挤」还是「丰富」、
> 七帧是不是同一个动作。范式见 `apps/clock_turn/tools/preview.py`。
>
...
> 与"下一步该先量什么"见 `docs/ROADMAP.md` §8。
...
- 🔴 **同屏「要看的东西」不超过两类**(2026-08-07 pipe_garden 否决理由,ROADMAP §7)。上一条只拦得住「太小」,拦不住**「太挤」**——每个对象单独都 ≥64px、加起来仍可能是一屏噪声。对照三强:tilt_maze = 球 + 墙,busy_knobs = 8 个格子,chick_pour = 一群鸡 + 一个窝。画面自检问一句:**孩子需要同时盯住几类东西?超过两类就砍。** 背景/装饰不计入,「会变、要判断」的才算。
...
- 🔴 **主循环收尾一律 `core2_sleep_pace(&last, delay_ms)`,不许裸 `vTaskDelayUntil`**:后者
  逾期立刻返回却**不清账**,单帧超时(渲染重/唤醒切电/抢 LVGL 锁)会攒成无上限的时间
  欠债;画面一变便宜就零延时连跑几百上千帧还债 → 物理 dt 写死 1/60,表现为「玩到一半
  全场对象突然集体加速」几秒后自愈(速度封顶拦不住,封的是仿真速度不是帧率)。
  2026-08-18 chick_pour 实证,六个工程已统一;机制见 `components/core2_sleep/README.md`。
- 🔴 **固定 dt 的物理,一次循环只跑一步 = 游戏速度 ≡ 实际帧率**(同日 chick_pour 第二刀):
  上一条只保证"不快进",不保证"不慢放"。渲染重的时候循环掉到 45Hz,整局就是 0.75 倍慢放;
  屏上对象变少(归家/被吃/出场)帧率爬回 60Hz,剩下的就"越玩越快"——**看着像手感 bug,
  其实是帧预算 bug**。修法即 §4 早写明的"逻辑可多步、画面丢帧":按真实流逝时间攒时间片、
  一次循环补跑 N 步物理、画面只更新一次,N 封顶(超出的时间丢掉,别滚雪球),休眠时累加器
  清零。先例 `apps/chick_pour/main/game_state.c`(`play_step`/`play_tick` + `PHYS_MAX_STEPS`)。
  ⚠️ 排查时**别先怀疑物理参数**:速度封顶封的是 px/秒,拥挤/分离也不压速度(主机链真身
  实测过),能让墙钟速度变的只有帧率。
- **关卡/内容手工编排 + 加载时校验**(如 BFS 可解性),别用纯随机(tilt_maze 见 `apps/tilt_maze/SPEC.md` §4.1/§19)。
- 🔴 **"校验失败就回退安全值"的护栏,自己写错时是静默的**:功能退化成基线(永远第 1 关 /
  永远同一张版面),屏幕上与「设计如此」分辨不出来,且**照样 0 warning、照样跑得好好的**。
  所以每道这类护栏都要配:**① 开机自检打一行日志**(`图纸自检:3/3 通过`),**② 主机可
  直接编译真身的回归测试**——把校验逻辑抽成不依赖 LVGL/BSP 的独立 .c(先例
  `apps/chick_pour/main/layout.c` + `tools/verify_host.sh`)。**只有"另一种语言的平行翻译"
  不算**(preview.py 的 Python 版一直是过的,C 版三张图纸全挂,只有实机暴露)。
  配套的排查纪律:**实机现象先要串口日志再改代码**——chick_pour 这条为省这一步白烧了两轮板
  (经过见 `apps/chick_pour/README.md`「'玩多久都只有一张后院'的真因」)。
