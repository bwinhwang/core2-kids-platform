...
- `ledstrip_fx` 新增 5 特效直接把灯珠索引序 0..9 当"环序"用,没做物理左右重映射;哪天觉得扫向反了,把 `ledstrip_fx.c` 里的索引序表取反即可(一行改)。
...
✅ **趣味增量第二批(2026-07-08)实机验收通过**(小脸活化 / 小鸟访客 / 图案差异化 / 夜晚音色 + 星星 / 和弦彩蛋;`apps/tilt_maze` 联动验证 `ledstrip_fx` 改动无回归)。
