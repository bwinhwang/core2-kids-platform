// slingshot_feed —— 传输/绑定层实现(形状照抄 bus_link.c 的 scan_bus/poll_joy/node_rgb/
...
static const char *TAG = "sling_link";
...
static bool poll_joy(bool draw)
{
    ...
    // 自适应回中(抄 busy_bus 定案理由:机械回中误差/温漂,固定死区盖不住;只在回中带内
    // 才慢慢拉中心,超出回中带说明真在推,那一路轴暂停校正)。
...
// ── 主任务(POLL_HZ,tuning.h 有偏离 20Hz 惯例的说明)─────────────────────
static void game_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (s_prev_stage == CORE2_SLEEP_DEEP && stage != CORE2_SLEEP_DEEP) {
            s_joy_id = 0;
            scan_bus(false);
            sling_game_sync_attach();
            sling_game_reset_after_wake();   // 弹弓复位待命,不吞已喂饱进度(SPEC §10)
        }
        s_prev_stage = stage;
        ...
        if (stage != CORE2_SLEEP_DEEP) {
            ...
            if ((s_joy_id != 0) != prev_bound) sling_game_sync_attach();
            ...
            if (stage == CORE2_SLEEP_AWAKE) sling_game_tick();
        }
...
void sling_link_start(void)
{
    ...
    xTaskCreate(game_task, "sling_link", 5120, NULL, 5, NULL);
}

// ── 游戏层输入 getter(应用侧自算帧间 delta/边沿,这里只暴露读数)────────────
bool  sling_link_joy_attached(void) { return s_joy_id != 0; }
float sling_link_joy_x(void)        { return s_joy_nx; }
float sling_link_joy_y(void)        { return s_joy_ny; }
bool  sling_link_joy_button(void)   { return s_joy_btn; }
...
void sling_link_hue2rgb(int h, uint8_t *r, uint8_t *g, uint8_t *b) { hue2rgb(h, r, g, b); }
