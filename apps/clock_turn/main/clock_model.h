...
//   时针角度 = t * 0.5          // MIN_PER_STEP=15 → 一步 7.5°,12h 走 48 步;
//                               // 仍是连续爬行、不吸附整点(§0.1 知识点 2 的关键)
//   分针角度 = (t % 60) * 6.0   // MIN_PER_STEP=15 → 一步 90°,一圈 4 步(12/3/6/9)
// 0° = 12 点钟方向,顺时针为正。
// ⚠️ 公式本身与 MIN_PER_STEP 无关(t 是分钟数,不是步数);上面括号里的"一步多少度"
//    只是按当前 MIN_PER_STEP=15 换算出来的说明,改步长时更新注释即可,别改公式。
#pragma once
 ...
 *  ⚠️ 2026-08-10 MIN_PER_STEP 改 15 后,**转出来的 t 恒满足本条件**(恒真),落格反馈里的
 *  "普通格"档因此不再出现——三档区分退化成整点/半点/一刻三级亮度。这是步长变粗的自然
 *  结果,不是 bug;函数本身保持通用(主机单测/将来调细步长时仍需要它),别顺手删。 */
bool clock_model_is_quarter(int t);

/**
 * @brief MODE_QUIZ 随机出题(SPEC §5.5,用户拍板豁免根 CLAUDE.md §11 的手工编排铁律)。
 *
 * 题池 = 粒度 grain_min 的全部时刻(默认 15 → 48 个),按 rand_u32 均匀映射抽取,
