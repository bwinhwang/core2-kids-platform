...
#define BALL_R          7.0f     // 球半径(px);直径 14,20px 窄走廊留 6px 余地(16×12 网格,2026-07-02)
#define GOAL_R          13.0f    // 到家判定半径(px):球心须实质进入家格(cell=20,不会隔墙误判)
#define STAR_R          7.0f     // 星(收集物)半径(px):渲染尺寸与拾取判定共用
...
#define HAZARD_SPEED    44.0f    // 直线怪往返速度(px/s);远低于 VEL_MAX(220),留出"能躲开"的窗口
#define HAZARD_DWELL_MS 600      // 直线怪走到端点后停留多久再回头(可预判的节奏,不是无休止扫)
// 绕圈怪(2026-08-03 七写)单列一档速度:它不停、不掉头,节奏全靠"一圈多久"来预判。
// 8 格方形环周长 160px → 一圈 3.6s;12 格环 240px → 5.5s;L6 的 20 格环 400px → 9.1s。
// 嫌"等太久才轮到它让开"就调快,嫌"来不及穿"就调慢——调这个不影响直线怪。
#define HAZARD_LOOP_SPEED 44.0f  // 2026-08-03 用户"再慢一丢丢":两档同步 50→44(-12%)
...
// 失败定格:踩陷阱/撞怪后先演完"死"再重进本关(2026-08-18)。原来是同一帧立刻重置,
// 失败没有存在感——孩子只看到球突然回到起点。
// 🔴 必须 > render.c 里最晚结束的那条(红框 FB_BAR_DELAY+FB_BAR_MS = 150+740 = 890ms),
// 否则 render_load_level 的 fail_fx_clear 会把没演完的东西直接删掉、演出被拦腰砍断。
#define FAIL_HOLD_MS        1050         // 死亡演出定格时长(到重进本关)
