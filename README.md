# Core2 游戏 + IoT 评估平台(多 App 卡带机)

本仓库是一台"卡带机":**factory 分区常驻 launcher 选择页,6 个 ota 槽各放一个
独立 app bin(ota_0~3 四张幼儿游戏、ota_4~5 两张 IoT 评估台),改哪个 app 只重编+单刷它自己
(~600KB,launcher 与其它 app 分毫不动)**。机制见 `components/app_slot/README.md`,烧录对照
`tools/flash_map.md`。

> **2026-07-17 平台转向**:早间一度整体转向 IoT 评估台、6 张幼儿游戏卡带全部下线;**当晚
> 用户修订为「游戏与 IoT 评估台共存」**——四张已实现游戏(tilt_maze/busy_knobs/chick_pour/
> chain_lab)从删除前的提交恢复重新上线,fish_pond/pipe_garden 当时仅有 SPEC 未实现不恢复。
> 硬件平台不变,新增重心是 **IoT 硬件评估**(外设/单元评估、功耗/系统评估)。决策与三个
> Phase 见 `docs/ROADMAP.md`(完整过程含当晚修订见 git 历史)。
...
1. **可复用平台(核心价值)**:Core2 v1.0 + M5GO Bottom2 的外设层已实机调通一次,
   ...
   **做新 APP = `tools/new_app.sh <名字>` 在 `apps/` 下生成工程 → 写逻辑 → 单刷进一个
   预留槽。** 复用指南:**`docs/platform/BSP_GUIDE.md`(新应用必读)**。
2. **四张游戏卡带**(`apps/{tilt_maze,busy_knobs,chick_pour,chain_lab}`,ota_0~3):幼儿掌机
   时期的 as-built 遗产,原样保留。
3. **两张评估卡带**:「外设/单元评估台」(`apps/unit_bench`,ota_4)扫描 PORT.A I2C / Chain
   菊花链,识别已知单元、展示数值+趋势图、支持热插拔与标定持久化,它同时是平台组件的用法
   参考;「功耗/系统评估台」(`apps/power_lab`,ota_5)评估 AXP192 电压/电流与休眠功耗。
...
> `ui_kit`/`data_log`);四张幼儿游戏卡带当天先删后又恢复(见 `docs/ROADMAP.md` §0"晚间
> 修订"),完整过程见 git 历史。
...
idf.py -C apps/unit_bench build              # 编一个 app(游戏或评估台都一样)
...
python -m esptool --chip esp32 -p <PORT> write-flash 0x990000 apps/unit_bench/build/unit_bench.bin
                                            # 日常:单刷一个槽(launcher/其它 app 不动)
```

🔴 **app 工程严禁 `idf.py flash`**——会烧到 factory 偏移**覆盖 launcher**(误刷用全量刷恢复)。

改 `sdkconfig.platform` / 工程 `sdkconfig.defaults` 后:对应工程 `rm -f sdkconfig` → fullclean → build;
**改 `partitions.csv` = 所有分区错位,全量刷 + 所有槽重刷**。
...
| 路径 | 作用 |
|---|---|
| `partitions.csv` | ★ 分区表唯一真源(factory=launcher + ota_0~5 六槽;定案冻结) |
| `sdkconfig.platform` | 共享 sdkconfig 默认值(各工程 CMakeLists 引入) |
| `launcher/` | factory 分区常驻的卡带机选择页(上电入口,数据驱动渲染各槽点击进入) |
| `apps/tilt_maze/` | 游戏 app:倾斜迷宫(ota_0;IMU 倾斜) |
| `apps/busy_knobs/` | 游戏 app:忙碌旋钮(ota_1;Chain Encoder) |
| `apps/chick_pour/` | 游戏 app:喂小鸡(ota_2;IMU 倾斜) |
| `apps/chain_lab/` | 游戏 app:吊臂抓取(ota_3;Chain Joystick) |
| `apps/unit_bench/` | 评估 app:外设/单元评估台(ota_4;PORT.A I2C + Chain 扫描/详情/CSV 导出) |
| `apps/power_lab/` | 评估 app:功耗/系统评估台(ota_5;AXP192 遥测 + 负载矩阵 + 休眠演练) |
