...
## 🔴 已知风险:BtnB/BtnC 分界只剩 7px

2026-08-02 实测**按中圈落在 x=221 y=279**,与注释里记的中圈中心 187 差了 34px,离 BtnC 分界
`X_SPLIT_BC=228` 只剩 **7px** —— 手指再右一点就落进 BtnC 区,而 BtnC 长按 = **关机**。
误触代价最大的一个键紧挨着最常按的那个,分界值该按实测重标(见上「怎么标」)。
截图那条路已端到端验过(BtnB + `screenshot.py --watch` 连存两张,游戏全程不中断)。
