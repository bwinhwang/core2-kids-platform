...
## 底层原语(`power_monitor` 等组件用)

`core2_power_read_regs(reg, buf, len)`——连续读 len 个寄存器,复用本组件已绑定的 AXP192
设备句柄。2026-07-17 平台转向新增,给 `power_monitor` 直读电池/VBUS 电压电流、库仑计等
ADC 寄存器用,避免重复 `i2c_master_bus_add_device`。AXP192 是普通寄存器寻址芯片,不是
8Encoder 那类 MCU 从机,组合读(repeated-start)安全。
