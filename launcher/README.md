# launcher — 游戏 + IoT 评估台选择页(as-built)

> 本文件是 **launcher 工程 + 多 App 分区机制的竣工记录**。**占 factory 分区**。多 App 单刷纪律的
> 速查见根 `CLAUDE.md` §9;分区偏移权威表见 `tools/flash_map.md`。
>
> **2026-07-17 平台转向**:原「幼儿游戏卡带机选择页」重写为**数据驱动**的选择页——不再需要为
> 每个新 app 手绘图标分支 + 重刷 launcher,详见下方"数据驱动渲染"。数据驱动的直接好处是当晚
> 「游戏与 IoT 评估台共存」修订(四张游戏恢复上线)对 launcher **零改动**——它本就不认卡带
> 类型,只读 `esp_app_desc_t` 渲染卡片。

## 仓库形态:游戏 + 评估卡带机

factory 分区常驻 **launcher 选择页**(本工程),6 个 ota 槽各放一个独立 app bin(ota_0~3 四张
游戏、ota_4~5 两张 IoT 评估台)。**改一个 app 只重编+单刷它自己(~600KB)**,launcher 与其它
app 零风险。
...
|---|---|---|---|
| factory | — | launcher | — |
| ota_0 | 0x190000 | tilt_maze(游戏) | IMU 倾斜 |
| ota_1 | 0x390000 | busy_knobs(游戏) | Chain Encoder |
| ota_2 | 0x590000 | chick_pour(游戏) | IMU 倾斜 |
| ota_3 | 0x790000 | chain_lab(游戏) | Chain Joystick |
| ota_4 | 0x990000 | unit_bench(评估台) | PORT.A I2C + Chain |
| ota_5 | 0xB90000 | power_lab(评估台) | AXP192 遥测 |

> 偏移以 `tools/flash_map.md` 为准。2026-07-17 早间一度转向 IoT 独占 ota_0/1、当晚修订为
> 游戏与 IoT 评估台共存后重排为上表,完整过程见 git 历史(`git log -- launcher/README.md`)。
