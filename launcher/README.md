...
暖色卡带架(3×2 大图标,空槽灰显但点了也有回应)+ 吉祥物浮动;点图标 → 校验镜像 → 重启进游戏(~2-3s 黑屏,物理卡带机固有体验);久置走 core2_sleep 两级省电,**再久置(DEEP 满 10min)自动关机**(2026-08-12 省电批,
`docs/ROADMAP.md` §8)——同一批还加了右上角电量指示(家长仪表)和"休眠时删掉吉祥物无限
动画"的 stage 门控:launcher 是设备待机时最常停留的页面,不门控就等于黑屏后还在按动画帧
重绘 + flush。
...
✅ **实机验收通过**:点图标进游戏 / Home 回 launcher(仅 tilt_maze 接了家长菜单)/
断电重启回 launcher / 单刷某个槽不动 launcher。
